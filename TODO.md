...
S0（開発環境整備 → E2E 環境構築）
  ↓
S1（SimpleCov → users/sessions → User モデル → 認証画面）
  ↓
S2（サインアップ → パスワードリセット → アカウント設定）
  ↓ [deploy] 認証フロー + [e2e] 認証 E2E
  ↓
S3（カテゴリ） → S4（支払方法）
  ↓ [deploy] カテゴリ・支払方法確認
  ↓
S5（CSV 基盤） → S6（CSV 画面）
  ↓ [deploy] CSV インポート確認 + [e2e] CSV E2E
  ↓
S7（明細基盤） → S8（明細画面）
  ↓ [deploy] 明細操作確認 + [e2e] 明細 E2E
  ↓
...
- [ ] CLI コマンドによる取り込み（`rails import:run`、対象ディレクトリの CSV を一括処理し処理済みファイルを移動）
- [ ] フォルダウォッチャー（常駐プロセスで対象フォルダを監視し CSV 設置で自動取り込み）

---

## フェーズ5: 認証・管理画面

**目標**: セキュアな認証と管理者機能の実装。詳細は [AUTHENTICATION.md](docs/design/AUTHENTICATION.md) を参照

- [ ] パスキー認証（webauthn-rails）の追加（フェーズ1の Rails 8 Built-in を拡張）
- [ ] 管理画面（ユーザー管理・取り込み済みデータの確認修正・カテゴリ分類ルールの管理）
- [ ] `admin` フラグによる管理者権限制御
...
- [ ] 資産推移グラフ
- [ ] 他ユーザーとの比較（匿名）
- [ ] モバイル対応（モバイルアプリ化 / React Native）
- [ ] CSV フォーマットの多様性への対応（楽天カード以外のカード会社）
- [ ] カテゴリ分類の精度向上（機械学習）
