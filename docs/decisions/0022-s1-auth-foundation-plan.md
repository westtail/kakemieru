...
- **[#21 で対応済み] `admin` の権限昇格対策（HIGH）** — `RegistrationsController` の strong parameters は `email_address / password / password_confirmation` のみ permit し `admin` を含めない。加えて `User` に `attr_readonly :admin`（作成後の AR 経由変更を禁止）を追加。request spec で「`admin=true` を送っても false のまま作成される」ことを検証。#22 の更新系も admin を permit していない。
