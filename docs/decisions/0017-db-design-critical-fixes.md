...
明細（transactions）の有無で動作を分ける。

| 条件 | 動作 |
|---|---|
| 明細ゼロの支払方法 | 物理削除 |
| 明細ありの支払方法 | `archived_at` をセット（永続アーカイブ） |
| ユーザー退会 | users の CASCADE 削除で payment_methods も全削除（アーカイブ済みも含む） |

**アーカイブ時の UI 説明**
- 「この支払方法はアーカイブされました。過去の明細はそのまま残り、新しい取り込みや手動入力では選択できなくなります。」
- 復元ボタンを用意する（`archived_at` を NULL に戻す）

**廃止した方針**
- 「1週間後に完全削除バッチ」は廃止。RESTRICT 制約と矛盾しており実行不可能なため。

### 理由

- 明細がない場合は誤登録の可能性が高く、完全に消せる方がUXがよい
- 明細がある場合は過去の集計・履歴が壊れるためアーカイブで永続保持する
- ユーザー退会時は CASCADE で全削除されるため、アーカイブ済みレコードが残留することはない
