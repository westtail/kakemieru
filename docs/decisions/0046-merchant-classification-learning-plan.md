...
- **学習**: カテゴリを手動付与したとき（`categorize` / `categorize_all` / 明細編集の `update`）に、店舗名（`CategoryClassifier.normalize` で正規化）→ category_id を `source: "user_manual"` で upsert（`MerchantClassification.learn`）。未分類化したときはその店舗のマッピングを削除（`.forget`）。
